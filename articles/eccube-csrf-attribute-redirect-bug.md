---
title: "EC-CUBEプラグインのCSRF検証でログインページに飛ばされる罠と回避策【Symfony #[IsCsrfTokenValid]】"
emoji: "🔐"
type: "tech"
topics: ["eccube", "eccube4", "symfony", "php", "security"]
published: true
---
...
:::message
**この記事のポイント（TL;DR）**
- `#[IsCsrfTokenValid]` が失敗すると `AuthenticationException` が投げられ、ログインページにリダイレクトされることがある
- EC-CUBE コアは `isTokenValid()` メソッドで `AccessDeniedHttpException`（403）を投げるため影響なし
- Symfony PR #57622（8.1向け）で専用の `InvalidCsrfTokenException`（HTTP 403）に変わり、ログインリダイレクトが起きなくなる
- 現状の回避策：`#[IsCsrfTokenValid]` 属性の代わりに EC-CUBE の `isTokenValid()` を使う
:::
...
:::message alert
PR #57622 は **Symfony 8.1 向けのオープン中のPR** です。EC-CUBE 4.3（Symfony ^6.4）では現時点で使えません。EC-CUBE の Symfony アップグレード対応（PR #6686 で Symfony 7.4 への移行が進行中）後、さらに将来のバージョンで利用可能になる予定です。
:::
