...
試行回数を超えると「ログイン試行回数が多すぎます。」というエラーが表示されます。完全なアカウントロックアウトではなく、IP + ユーザー名の組み合わせに対するスロットリングです。

:::message
**スロットリングの限界について**

IP + ユーザー名の組み合わせが制限の単位になるため、攻撃者が IP を変えながら同一ユーザー名に対してパスワードを試行する「分散型ブルートフォース」は防ぎきれません。追加の対策として、二要素認証（2FA）の有効化や、Fail2ban などの IP レベルのブロック手段との併用を検討してください。
:::

ログイン失敗の履歴は `LoginHistoryListener` が Symfony 標準の `LoginFailureEvent` を購読して `LoginHistory` エンティティとして記録します（管理画面ファイアウォールのみ対象）。

### 購入フローの保護（eccube_rate_limiter.yaml）

EC-CUBE には独自の `eccube_rate_limiter.yaml` があります。4.3 のソースコードで確認できるのは以下の購入フロー関連ルートです。

| リミッター名 | 対象 | 上限 | 時間窓 |
|-------------|------|------|-------|
| `shopping_confirm_ip` | IP | 1024回 | 30分 |
| `shopping_confirm_customer` | ユーザー | 1024回 | 30分 |
| `shopping_checkout_ip` | IP | 1024回 | 30分 |
| `shopping_checkout_customer` | ユーザー | 1024回 | 30分 |

上限が 1024回/30分 と非常に高い値に設定されており、通常の利用では制限に引っかかりません。これは購入フロー中の正規リクエストを遮断しないよう余裕を持たせた設計と考えられます。

:::message alert
**本番環境の動作確認について**

開発環境（`APP_ENV=dev`）ではレート制限が完全に無効になります。ステージング環境でも `APP_ENV=prod` に設定しないとレート制限が機能しない点に注意してください。
:::
...
| 購入フロー保護 | `eccube_rate_limiter.yaml` で shopping_confirm/checkout に設定済み |
| `#[RateLimit]` アトリビュート | **Symfony 8.1 向けのため現時点では利用不可** |
| プラグインでの追加 | `RateLimiterFactory` を手動 DI して `consume()` を呼ぶ |

EC-CUBE 4.3 は `symfony/rate-limiter` を土台として、ログインスロットリングと購入フローへの保護をすでに実装しています。プラグインで独自にレート制限を追加したい場合は、`framework.rate_limiter` にリミッターを定義し、コントローラで `RateLimiterFactory` を DI するアプローチが最短経路です。
