...
GitHubの「Settings」→「Collaborators」→「Add people」から対象メンバーのGitHubアカウントを招待します。メンバーが招待を承認するとリポジトリをクローンできるようになります。

### 2. サーバーにSSH公開鍵を追加する（マネージャーのみ）

サーバーに直接SSHできる必要があるのはデプロイを行うマネージャーだけです。マネージャーを追加する場合は、その人の公開鍵をサーバーに追加します。

追加するメンバーのローカルマシンで公開鍵を確認します。

```bash
cat ~/.ssh/id_ed25519.pub
```

表示された公開鍵をサーバーの `~/.ssh/authorized_keys` に追記します。

```bash
ssh eccube-admin@<サーバーのIPアドレス>
echo "追加するメンバーの公開鍵" >> ~/.ssh/authorized_keys
```

追加後、そのメンバーのマシンから接続できることを確認します。
...
チームメンバーが退職した場合、役割に応じて対応が異なります。

| 役割 | GitHub削除 | SSH鍵削除 | 機密情報ローテーション |
|---|---|---|---|
| 一般メンバー | 必須 | 不要 | 不要 |
| マネージャー | 必須 | **必須** | 推奨 |

一般メンバーはGitHubへのアクセス権しか持っていないため、GitHub から削除するだけで十分です。一方、マネージャーはサーバーへのSSHアクセスを持っているため、**GitHub を削除してもSSH鍵が残っている限りサーバーに直接ログインできます**。SSH鍵の削除も必ず行ってください。
